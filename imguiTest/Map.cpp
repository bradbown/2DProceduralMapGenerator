...
{
	map = &sf::RectangleShape(sf::Vector2f(width, height));
...
void Map::Update(sf::RenderWindow& window, Input& input)
{
...
{
	//window.draw(*map);	
	ImGui::Begin("Map Window", NULL, window_flags);

	{
		ImGuiWindowFlags child_window_flags = ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove;
		ImGui::BeginChild("MapChild", ImVec2(ImGui::GetWindowContentRegionWidth() * 0.75f, (ImGui::GetWindowHeight()) - 20), true, child_window_flags);
		if (mapVisible)
		{
			ImGui::Image(heightMap);
		}
		ImGui::EndChild();
	}
	...
	{
		ImGuiWindowFlags child_window_flags = ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove;
		ImGui::BeginChild("MenuChild", ImVec2(ImGui::GetWindowContentRegionWidth() * 0.24f, (ImGui::GetWindowHeight()) - 20), true, child_window_flags);
		ImGui::Text("Menu");
...
void Map::initMap()
{
	...
	//std::vector<float> falloffMap = GenerateFalloffMap(32);
